...
class Indexer:
    ...
    def process(self):
        if self.url_manager.has_more_urls():
            ...
            print(
                "URL " + url + " is indexed: " + str(is_indexed) + " Status: " + status
            )
    ...
    def proxy_request(self, url, **kwargs):
        ...
        while True:
            ...
            if current_proxy is None:
                ProgressManager.update_progress("All given proxy failed, using regular requests")
        ...
        ProgressManager.update_progress(
            "No working proxies left! Using normal request..."
        )
        self.proxy_manager.update_proxy()
